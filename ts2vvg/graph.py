...
def __criteria_vvg(series_a: np.ndarray,
    ...
    """
    proj_aa = norm_a
    proj_ab = __projection_vectors_vvg(series_a, series_b, norm_a)
    proj_ac = __projection_vectors_vvg(series_a, series_c, norm_a)
    time_frac = (time_b - time_c) / (time_b - time_a)
    vg = proj_ab + (proj_aa - proj_ab) * time_frac
    return proj_ac < vg


def build_graph(series_a: np.ndarray, series_b: np.ndarray) -> dict:
        ...
        for s, sample_s in enumerate(all_samples[i + 1:], start=i + 1):
            if s == i + 1:
                ...
                for t, sample_t in enumerate(all_samples[i + 1:s], start=i + 1):
                    if __criteria_vvg(sample_i, sample_s, sample_t, i, s, t, norm_a):
                        adjacency_list[i].append(s)
                        adjacency_list[s].append(i)
                        break
